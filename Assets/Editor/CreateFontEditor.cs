...
{
    const string FontsPath = "Assets/Casual/Bundle/Fonts/";
    const string FontMaterialPath = "Assets/Casual/Bundle/Fonts/Materials/";
    const string FontTexturePath = "Assets/Casual/Bundle/Fonts/Textures/";

    [MenuItem("Assets/Tools/Make Custom Font", priority = 51)]
    static void CreateFont()
    {
        Object obj = Selection.activeObject;
        string fntPath = AssetDatabase.GetAssetPath(obj);
        if (!Path.GetExtension(fntPath).Equals(".fnt"))
        {
            Debug.LogError("格式不是.fnt，无法创建自定义字体！");
            return;
        }

        string fontName = obj.name;

        Font customFont = null;
        string customFontPath = FontsPath + fontName + ".fontsettings";
        ...
        string customMaterialPath = FontMaterialPath + fontName + ".mat";
        string textureFilename = FontTexturePath + fontName + ".png";

        if (!File.Exists(customMaterialPath))
        {
        ...
        {
            customMaterial = AssetDatabase.LoadAssetAtPath<Material>(customMaterialPath);
        }

        Texture tex = AssetDatabase.LoadAssetAtPath(textureFilename, typeof(Texture)) as Texture;
        customMaterial.SetTexture("_MainTex", tex);

        customFont.material = customMaterial;
